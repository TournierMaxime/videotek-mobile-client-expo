...
/*       const refreshedState = await refresh(config, {
      ...
      if (users.users && users.users.length > 0) {
        ...
        const login = await dispatch(loginUser({ userId }))

        if (login.errors) {
          return setGoogleMessage({ error: login.errors[0].message })
        }
        ...
        if (register.error) {
          return setGoogleMessage({ error: register.error.response.data.errMsg })
        }

        const login = await dispatch(
          loginUser({ userId: register.user.userId })
        )

        if (login.errors) {
          return setGoogleMessage({ error: login.error.response.data.errMsg })
        }
